...
def check_window_zoom(t):
    ...
    PEAK_EDIT_MAX_WINDOW_T = 0.5  # smaller than this doesn't make sense for ECG data.

    if gb["WINDOW_T"] > PEAK_EDIT_MAX_WINDOW_T:

        # First zoom in
        update_window(0.8 * PEAK_EDIT_MAX_WINDOW_T / gb["WINDOW_T"], t)
...
def load_channels(chans):
    ...
    for c in chans:
        ...
        gb["data"][c] = {"hdr": hdr, "vals": vals, "t": t}
...
def update_window(fact, around_t):
    ...
    if not around_t:
        around_t = gb["tstart"] + gb["WINDOW_T"] / 2
    t_prop = (around_t - gb["tstart"]) / gb[
        "WINDOW_T"
    ]  # get at what proportion of the window that time point is located
    gb["WINDOW_T"] *= fact
...
def redraw():
    ...
    for i, chan in enumerate(gb["channels"]):
        ...
        fromi, toi = gb["bio"].get_closest_sample(chan, tmin), gb[
            "bio"
        ].get_closest_sample(chan, tmax)

        # tsels = (t>=tmin) & (t<=tmax)

        gb["cursor"][i] = ax.axvline(
            x=gb["cursor.t"], lw=1, color="blue", alpha=0.9, zorder=99999
        )
        ...
        ax.plot(x, y, pch, label="cleaned", zorder=-10, color=gb["COLORS"][chan])
...
def main(bio):
    ...
    gb["WINDOW_SHIFT_T"] = (
        0.2  # proportion of the window to shift when we go back and forth in time
    )
    ...
    button_wid = tkinter.Button(
        master=navf, text="+", command=window_wider, font=bigfont
    )
    button_narr = tkinter.Button(
        master=navf, text="-", command=window_narrower, font=bigfont
    )
    button_wid.grid(column=0, row=0, padx=0, pady=10)
    button_narr.grid(column=1, row=0, padx=0, pady=10)

    button_back = tkinter.Button(
        master=navf, text="<", command=back_in_time, font=bigfont
    )
    button_forw = tkinter.Button(
        master=navf, text=">", command=forward_in_time, font=bigfont
    )
    ...
    for chan in gb["channels"]:
        ...
        viewmenu.add_checkbutton(
            label=chan, variable=v, onvalue=1, offvalue=0, command=c
        )
    ...
    for chan in gb["channels"]:
        onlyview.add_command(
            label='"{}"'.format(chan), command=lambda x=chan: show_channels([x])
        )
    viewmenu.add_cascade(label="Show only", menu=onlyview)
    viewmenu.add_command(
        label="Show all", command=lambda x=gb["channels"]: show_channels(x)
    )
    viewmenu.add_separator()
    v = IntVar()
    v.set(1)
    viewmenu.add_checkbutton(
        label="Show markers", variable=v, onvalue=1, offvalue=0, command=redraw_all
    )
    gb["showmarkers"] = v
