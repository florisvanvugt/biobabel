...
def get_file(descr=""):
    ...
    parser.add_argument(
        "filename",
        nargs="?",
        help="The file to open. If no file is provided, a window will open to ask you to select a filename.",
    )
    parser.add_argument(
        "-v",
        "--version",
...
def ask_bio_file():
    ...
    filetypes = biobabel.io.get_compatible_file_types() + [("All files", "*.*")]

    fname = fd.askopenfilename(
        title="Select your recording", initialdir=".", filetypes=filetypes
    )
...
def tohdf5():
    ...
    bio = get_file(
        "Convert a file to the biobabel-native HDF5 format."
    )  # attempt to get a file name to work on
...
def split():
    ...
    """
    bio = get_file(
        "Split file into smaller files along time markers."
    )  # attempt to get a file name to work on
    ...
    if len(ts) < 3:
        print(
            "Only {} time points (including beginning and ending). Not enough to split."
        )
        sys.exit(0)
...
def merge():
    ...
    parser = argparse.ArgumentParser(
        description="Merge a series of physiology files by concatenating them in time."
    )
...
def html_report():
    ...
    parser = argparse.ArgumentParser(
        description="Produce a quick overview of a range of physiology files."
    )
    parser.add_argument(
        "filename", nargs="+", help="The files to include in the report."
    )
    parser.add_argument(
        "-v",
        "--version",
