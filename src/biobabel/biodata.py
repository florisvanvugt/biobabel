...
class Biodata:
    ...
    def add_channel(self, hdrdat):
        ...
        """
        hdr, dat = hdrdat
        newid = hdr["id"].replace(
            "/", "_"
        )  # slashes don't work because HDF5 gets confused
    ...
    def get_duration(self, chid=None):
        ...
        if (
            chid == None
        ):  # If no channel is given, return maximum duration of all channels
    ...
    def summarize_channel(self, chid):
        ...
        ret = "channel {} [ modality {} {}] {} samples @ {:.1f} Hz = {:.1f} s".format(
            chid, hdr.get("modality", "N/A"), units, nsamp, SR, dur
        )
    ...
    def plot(
        self, channels=None, figsize=(12, 7), timerange=None, show=True, markers=True
    ):
        """
        ...
        f, axs = plt.subplots(
            len(chans), 1, sharex=True, figsize=figsize, squeeze=False
        )
    ...
    def html_report(self):
        ...
        for trange in tranges:
            ...
            html += '<p><img src="data:image/jpeg;base64,{}"></p>'.format(
                b64.decode("utf-8")
            )
            plt.close()
    ...
    def crop(self, tfrom=None, tend=None):
        ...
        """
        if tfrom == None and tend == None:
            return  # otherwise it doesn't really make sense eh?
        if tfrom == None:
            tfrom = -np.inf
        if tend == None:
            tend = np.inf
        ...
        for m in self.markers.keys():
            mrk = self.markers[m]
            self.markers[m] = [t - tfrom for t in mrk if t > tfrom and t < tend]
    ...
    def drop(self, what):
        ...
        if isinstance(what, str):
            ...
            self.channels = [
                (hdr, vals) for (hdr, vals) in self.channels if hdr["id"] != what
            ]

        if isinstance(what, list):
            # Assume that this is a list of IDs of the channel we want to drop so drop them
            self.channels = [
                (hdr, vals) for (hdr, vals) in self.channels if hdr["id"] not in what
            ]
    ...
    def select(self, what):
        ...
        if isinstance(what, list):
            # Assume that this is a list of IDs of the channel we want to drop so drop them
            self.channels = [
                (hdr, vals) for (hdr, vals) in self.channels if hdr["id"] in what
            ]
